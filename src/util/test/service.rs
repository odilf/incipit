...
pub struct Service<T = Server> {
    ...
    /// status code.
    pub handler: fn(&str) -> Result<String, u16>,
...
impl StoppedService {
    pub async fn start(self) -> eyre::Result<Service> {
        let server = Server::start(([127, 0, 0, 1], self.config.port).into(), self.handler).await?;
...
fn service2() -> StoppedService {
    Service {
        handler: |path| Ok(format!("Hello path: {path}")),
        config: ServiceConfig {
            port: 9423,
